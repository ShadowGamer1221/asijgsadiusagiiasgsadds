...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if(!message.member.hasPermission("BAN_MEMBERS")) {embed.setDescription("You need the `BAN_MEMBERS` permission to use this command!"); return message.lineReply(embed)}
        if(!message.guild.me.hasPermission("BAN_MEMBERS")) {embed.setDescription("I do not have permission to ban people! Please give me the `BAN_MEMBERS` permission!"); return message.lineReply(embed)}
        ...
        if(!mention) {embed.setDescription("You did not provide anyone to ban!"); return message.lineReply(embed)}
        let user = message.guild.member(mention)
        if(user) mention = user
        let banned
        try {banned = await message.guild.fetchBan(mention.id)} catch (err) {}
        if(banned) {embed.setDescription("That person is already banned!"); return message.lineReply(embed)}
        if(mention.id === message.author.id) {embed.setDescription("You cannot ban yourself!"); return message.lineReply(embed)}
        if(mention.roles && !message.member.roles.highest.position > mention.roles.highest.position) {embed.setDescription("Your highest role is too low to ban this member!"); return message.lineReply(embed)}
        if(mention.roles && !mention.bannable) {embed.setDescription("I cannot ban this member! Please insure my role is higher than who you mentioned!"); return message.lineReply(embed)}
        let good = true
